// Build an embeddable Neural Network visualization as a pure ES module
// Usage:
//   import { mountNeuralNetwork } from './assets/js/neuralNetwork.js'
//   const instance = await mountNeuralNetwork(document.getElementById('container'))
//   instance.destroy() // to cleanup
//
// The module programmatically creates UI elements, injects styles, and mounts a Three.js scene.
...
function injectStylesOnce() {
  const styleId = "nn-styles";
  if (document.getElementById(styleId)) return;
  const style = document.createElement("style");
  style.id = styleId;
  style.textContent = `
    * { box-sizing: border-box; }
    .nn-neural-network-container { position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: -1; }
    .nn-neural-network-container canvas { display: block; width: 100%; height: 100%; cursor: pointer; position: absolute; top: 0; left: 0; z-index: 1; pointer-events: auto; touch-action: none; -ms-touch-action: none; }

    .nn-ui-panel { position: absolute; backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); background: rgba(0,0,0,.7); border-radius: 12px; border: 1px solid rgba(255,120,50,.3); box-shadow: 0 4px 20px rgba(0,0,0,.5); z-index: 10; padding: 15px; color: #eee; font-family: 'Inter', sans-serif; pointer-events: none; }

    .nn-instructions { top: 20px; left: 20px; font-size: 14px; line-height: 1.5; max-width: 280px; }
    .nn-instruction-title { font-weight: 600; margin-bottom: 6px; font-size: 15px; }

    .nn-theme-selector { top: 20px; right: 20px; display: flex; flex-direction: column; gap: 12px; max-width: 150px; }
    .nn-theme-selector-title { font-weight: 600; font-size: 15px; margin-bottom: 2px; }
    .nn-theme-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
    .nn-theme-button { width: 36px; height: 36px; border-radius: 8px; border: 2px solid rgba(255,255,255,.3); cursor: pointer; transition: transform .2s, border-color .2s; outline: none; overflow: hidden; background: transparent; pointer-events: auto; }
    .nn-theme-button:hover, .nn-theme-button:focus { transform: scale(1.05); border-color: rgba(255,255,255,.7); }
    .nn-theme-button.active { transform: scale(1.05); border-color: rgba(255,255,255,.9); box-shadow: 0 0 10px rgba(255,200,150,.6); }

    .nn-theme-1 { background: linear-gradient(45deg, #4F46E5, #7C3AED, #C026D3, #DB2777); }
    .nn-theme-2 { background: linear-gradient(45deg, #F59E0B, #F97316, #DC2626, #7F1D1D); }
    .nn-theme-3 { background: linear-gradient(45deg, #EC4899, #8B5CF6, #6366F1, #3B82F6); }
    .nn-theme-4 { background: linear-gradient(45deg, #10B981, #A3E635, #FACC15, #FB923C); }

    .nn-density-controls { margin-top: 8px; display: flex; flex-direction: column; gap: 8px; }
    .nn-density-label { font-size: 13px; display: flex; justify-content: space-between; }
    .nn-density-slider { width: 100%; appearance: none; height: 4px; border-radius: 2px; background: rgba(255,120,50,.3); outline: none; cursor: pointer; pointer-events: auto; }
    .nn-density-slider::-webkit-slider-thumb { appearance: none; width: 14px; height: 14px; border-radius: 50%; background: rgba(255,120,50,.8); cursor: pointer; transition: transform .1s, background .1s; }
    .nn-density-slider::-webkit-slider-thumb:hover { transform: scale(1.1); background: rgba(255,140,50,1); }
    .nn-density-slider::-moz-range-thumb { width: 14px; height: 14px; border-radius: 50%; background: rgba(255,120,50,.8); cursor: pointer; border: none; transition: transform .1s, background .1s; }
    .nn-density-slider::-moz-range-thumb:hover { transform: scale(1.1); background: rgba(255,140,50,1); }

    .nn-control-buttons { position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); display: flex; gap: 15px; z-index: 10; background: rgba(0,0,0,.6); padding: 10px 15px; border-radius: 10px; border: 1px solid rgba(255,120,50,.2); }
    .nn-control-button { background: rgba(255,120,50,.2); color: #eee; border: 1px solid rgba(255,150,50,.3); padding: 8px 15px; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 600; transition: background-color .2s, transform .1s; white-space: nowrap; min-width: 80px; text-align: center; font-family: 'Inter', sans-serif; pointer-events: auto; }
    .nn-control-button:hover, .nn-control-button:focus { background: rgba(255,120,50,.4); outline: none; }
    .nn-control-button:active { background: rgba(255,120,50,.6); transform: scale(0.95); }

    @media (max-width: 640px) {
      .nn-instructions { max-width: calc(100% - 40px); font-size: 13px; padding: 10px 15px; top: 10px; left: 10px; }
      .nn-instruction-title { font-size: 14px; }
      .nn-theme-selector { top: auto; bottom: 20px; right: 10px; left: auto; transform: none; max-width: 120px; padding: 10px; }
      .nn-theme-selector-title { font-size: 14px; }
      .nn-theme-button { width: 30px; height: 30px; }
      .nn-density-label { font-size: 12px; }
      .nn-control-buttons { bottom: 10px; gap: 10px; padding: 8px 10px; }
      .nn-control-button { padding: 6px 10px; font-size: 12px; min-width: 65px; }
    }

    @media (max-width: 400px) {
      .nn-theme-selector { flex-direction: column; align-items: center; max-width: none; width: calc(100% - 20px); left: 10px; right: 10px; bottom: 75px; }
      .nn-theme-grid { grid-template-columns: repeat(4, 1fr); width: 100%; justify-items: center; }
      .nn-density-controls { width: 80%; margin-top: 15px; }
      .nn-control-buttons { width: calc(100% - 20px); justify-content: space-around; }
    }
  `;
...
function buildUI(root) {
  ...
  container.className = "nn-neural-network-container";
  ...
  // Return only the essential elements for network interaction
  return {
    root: container,
    canvas: canvas,
    // themePanel: themePanel,  // Commented out
    // controls: controls       // Commented out
  };
}
...
function generateNeuralNetwork(formationIndex, densityFactor = 1.0) {
  ...
  function generateQuantumCortex() {
    ...
    for (const ringDist of ringDistances) {
      ...
      for (let i = 0; i < ringLayer.length; i++) {
        ...
        if (i % 4 === 0 && ringLayer.length > 5) {
          const jumpIdx =
            (i + Math.floor(ringLayer.length / 2)) % ringLayer.length;
    ...
    for (let r = 0; r < ringNodes.length - 1; r++) {
      ...
      for (let i = 0; i < connectionsCount; i++) {
        const innerNode =
          innerRing[Math.floor(Math.random() * innerRing.length)];
        const outerNode =
          outerRing[Math.floor(Math.random() * outerRing.length)];
  ...
  function generateHyperdimensionalMesh() {
    ...
    for (let d = 0; d < dimensions; d++) {
      ...
      for (let i = 0; i < nodesPerDimension; i++) {
        ...
        const biasedVec = new THREE.Vector3()
          .addVectors(
    ...
    for (let d = 0; d < dimensions; d++) {
      ...
      for (let layer = 0; layer < layers; layer++) {
        ...
        for (let i = startIdx; i < endIdx; i++) {
          ...
          for (
            let j = 0;
            j < Math.min(connectionsCount, nearbyNodes.length);
            j++
          ) {
          ...
          if (layer > 0) {
            const prevLayer = dimNodes
              .slice((layer - 1) * nodesPerLayer, layer * nodesPerLayer)
              .sort(
                (a, b) =>
                  node.position.distanceTo(a.position) -
                  node.position.distanceTo(b.position)
    ...
    for (let d1 = 0; d1 < dimensions; d1++) {
      for (let d2 = d1 + 1; d2 < dimensions; d2++) {
        ...
        for (let i = 0; i < connectionsCount; i++) {
          const n1 =
            dimensionNodes[d1][
              Math.floor(Math.random() * dimensionNodes[d1].length)
            ];
          const n2 =
            dimensionNodes[d2][
              Math.floor(Math.random() * dimensionNodes[d2].length)
            ];
    ...
    for (let i = 0; i < jumpConnections; i++) {
      ...
      const startNode =
        dimensionNodes[startDim][
          Math.floor(Math.random() * dimensionNodes[startDim].length)
        ];
      const endNode =
        dimensionNodes[endDim][
          Math.floor(Math.random() * dimensionNodes[endDim].length)
        ];
      if (!startNode.isConnectedTo(endNode)) {
        ...
        for (let j = 1; j < numPoints; j++) {
          ...
          const jumpNode = new Node(
            pos,
            Math.max(startNode.level, endNode.level),
            0
          );
  ...
  function generateNeuralVortex() {
    ...
    for (let s = 0; s < numSpirals; s++) {
      ...
      for (let c = 0; c < connections; c++) {
        ...
        const midPoint = new THREE.Vector3()
          .lerpVectors(start.position, end.position, 0.5)
          .multiplyScalar(0.7);
        const bridgeNode = new Node(
          midPoint,
          Math.max(start.level, end.level),
          0
        );
    ...
    for (let r = 0; r < ringLevels; r++) {
      ...
      const ringNodes = nodes.filter(
        (n) => n !== rootNode && Math.abs(n.position.y - height) < 2
      );
      ringNodes.sort(
        (a, b) =>
          Math.atan2(a.position.z, a.position.x) -
          Math.atan2(b.position.z, b.position.x)
      );
      if (ringNodes.length > 3) {
        for (let i = 0; i < ringNodes.length; i++) {
          ringNodes[i].addConnection(
            ringNodes[(i + 1) % ringNodes.length],
            0.5
          );
        }
      }
    ...
    const candidates = nodes
      .filter((n) => n !== rootNode && n.position.length() > 5)
  ...
  function generateSynapticCloud() {
    ...
    for (let i = 0; i < clusterNodes.length; i++) {
      for (let j = i + 1; j < clusterNodes.length; j++) {
        const dist = clusterNodes[i].position.distanceTo(
          clusterNodes[j].position
        );
    ...
    for (let i = 0; i < interClusterCount; i++) {
      ...
      const bridgePos = new THREE.Vector3().lerpVectors(
        cluster1.position,
        cluster2.position,
        0.3 + Math.random() * 0.4
      );
      ...
      const nearbyNodes = nodes.filter(
        (n) =>
          ...
          n !== cluster1 &&
          n !== cluster2 &&
          n.position.distanceTo(bridgePos) < 8
      );
    ...
    const outerNodes = nodes
      .filter((n) => n.distanceFromRoot > maxDist * 0.6)
      .sort(() => Math.random() - 0.5)
    ...
    for (const outerNode of outerNodes) {
      ...
      const innerNodes = nodes.filter(
        (n) => n.distanceFromRoot < maxDist * 0.4 && n !== rootNode
      );
  ...
  if (densityFactor < 1.0) {
    ...
    console.log(
      `Density Filter: ${originalNodeCount} -> ${nodes.length} nodes`
    );
  }
...
export async function mountNeuralNetwork(container, options = {}) {
  if (!container)
    throw new Error("mountNeuralNetwork: container element is required");
  ensureFontsInjected();
  injectStylesOnce();

  const config = { ...defaultConfig, ...options };

  // Build scoped UI within the provided container
  const { root, canvas } = buildUI(container);
  ...
  // COMMENTED OUT TO ALLOW MAIN PAGE SCROLLING
  controls3d.autoRotate = true;
  controls3d.autoRotateSpeed = 0.3;

  // Disable all controls to prevent scroll interference
  ...
  // Pulse uniforms for shader effects
  const pulseUniforms = {
    uTime: { value: 0.0 },
    uPulsePositions: {
      value: [
  ...
  // Event listeners for network interaction only
  // Re-enabled click for neural network interaction
  ...
  // Prevent scroll events while keeping clicks enabled
  // COMMENTED OUT TO ALLOW MAIN PAGE SCROLLING
  // renderer.domElement.addEventListener(
  //   "wheel",
  //   (e) => {
  ...
  function createNetworkVisualization(formationIndex, densityFactor = 1.0) {
    ...
    neuralNetwork.nodes.forEach((node) => {
      ...
      const indices = node.connections
        .slice(0, 3)
        .map((conn) => neuralNetwork.nodes.indexOf(conn.node));
  ...
  function updateTheme(paletteIndex) {
    ...
    nodesMesh.material.uniforms.uPulseColors.value.forEach((c, i) =>
      c.copy(palette[i % palette.length])
    );
    connectionsMesh.material.uniforms.uPulseColors.value.forEach((c, i) =>
      c.copy(palette[i % palette.length])
    );
  ...
  function triggerPulse(clientX, clientY) {
    ...
    if (raycaster.ray.intersectPlane(interactionPlane, interactionPoint)) {
      ...
      if (nodesMesh && connectionsMesh) {
        ...
        connectionsMesh.material.uniforms.uPulseTimes.value[lastPulseIndex] =
          time;
        const palette = colorPalettes[config.activePaletteIndex];
        const randomColor = palette[Math.floor(Math.random() * palette.length)];
        ...
        connectionsMesh.material.uniforms.uPulseColors.value[
          lastPulseIndex
        ].copy(randomColor);
      }
    }
  ...
  function destroy() {
    ...
    // Remove DOM subtree we added
    if (root && root.parentElement === container) {
  ...
  return {
    ...
    pause: () => {
      config.paused = true;
      // pausePlayBtn.textContent = "Play"; // Removed
      // controls3d.autoRotate = false; // Removed
    },
    play: () => {
      config.paused = false;
      // pausePlayBtn.textContent = "Pause"; // Removed
      // controls3d.autoRotate = true; // Removed
    },
    changeFormation: () => {
      // changeFormationBtn.click(); // Removed
    },
    updateTheme: () => {
      // updateTheme(config.activePaletteIndex); // Removed
    },
    resetCamera: () => {
      // resetCameraBtn.click(); // Removed
    },
    setDensity: (percent) => {
      const v = Math.max(20, Math.min(100, Math.round(percent)));
      // densitySlider.value = String(v); // Removed
      // densitySlider.dispatchEvent(new Event("input", { bubbles: true })); // Removed
    },
    get config() {
      return { ...config };
    },
