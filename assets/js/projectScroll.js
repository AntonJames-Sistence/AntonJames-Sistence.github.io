document.addEventListener("DOMContentLoaded", function () {
    ...
    function smoothScroll(left) {
        ...
        function step(currentTime) {
            ...
            if (elapsedTime < 500) { // Adjust the duration as needed
                imageContainer.scrollLeft = easeInOutQuad(elapsedTime, startScroll, endScroll - startScroll, 500);
    ...
    scrollRightButton.addEventListener("click", function () {
        if (!scrolling) {
            scrolling = true;
            smoothScroll(Math.min(imageContainer.scrollLeft + scrollAmount, imageContainer.scrollWidth - imageContainer.clientWidth));
        }
    });
});
